      ...
      function showGreetingPopup() {
        ...
        if (!hasPopupBeenShown()) {
          ...
          if (window.location.pathname === '/' || window.location.pathname === '/trikon/') {
            // Create the HTML structure for the popup.
            var popupHtml = '<div id="greeting-popup">' + '<div class="popup-content">' + '<p>' + greeting + '</p>' + '<button id="close-popup">Close</button>' + '</div>' + '</div>'; // Append the popup HTML to the body.

            $('body', context).append(popupHtml); // Center the popup on the screen.
            ...
            $('#close-popup').on('click', function () {
              popup.remove(); // Set a cookie to indicate that the popup has been shown.
...
function openWhatsApp() {
  ...
  var whatsappURL = 'https://api.whatsapp.com/send?phone=' + encodeURIComponent(adminNumber) + '&text=' + encodeURIComponent(message); // Open the WhatsApp URL in a new tab/window
